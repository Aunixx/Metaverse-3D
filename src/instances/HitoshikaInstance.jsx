...
import { HitoshikaPlotting } from "../plots/HitoshikaPlotting";

export function HitoshikaInstances({ temp = new THREE.Object3D() }) {
  const ref = useRef();
  ...
  var large = 0;
  useEffect(() => {
    for (let i = 0; i < HitoshikaPlotting.length; i++) {
      if (HitoshikaPlotting[i].type === "Small") {
        small++;
        temp.scale.set(0.35, 0.35, 0.35);
      } else if (HitoshikaPlotting[i].type === "Medium") {
        medium++;
        temp.scale.set(0.67, 0.67, 0.67);
      } else if (HitoshikaPlotting[i].type === "XL") {
        temp.scale.set(2, 2, 2);
      } else if (HitoshikaPlotting[i].type === "XXL") {
      ...
      temp.position.set(
        HitoshikaPlotting[i].position.x,
        HitoshikaPlotting[i].position.y,
        4.1
      );
      if (HitoshikaPlotting[i].commercial === true) {
      ...
      temp.rotation.set(0, 0, -HitoshikaPlotting[i].rotation.radian);
      temp.updateMatrix();
  ...
  return (
    <instancedMesh
      ref={ref}
      args={[null, null, HitoshikaPlotting.length]}
      // onClick={(e) => {
      //   e.stopPropagation();
