...
import { AzraqPlotting } from "../plots/AzraqPlotting";

export function AzraqInstances({ temp = new THREE.Object3D() }) {
  const ref = useRef();
  ...
  var large = 0;
  useEffect(() => {
    for (let i = 0; i < AzraqPlotting.length; i++) {
      if (AzraqPlotting[i].type === "Small") {
        small++;
        temp.scale.set(0.35, 0.35, 0.35);
      } else if (AzraqPlotting[i].type === "Medium") {
        medium++;
        temp.scale.set(0.67, 0.67, 0.67);
      } else if (AzraqPlotting[i].type === "XL") {
        temp.scale.set(2, 2, 2);
      } else if (AzraqPlotting[i].type === "XXL") {
      ...
      temp.position.set(
        AzraqPlotting[i].position.x,
        AzraqPlotting[i].position.y,
        4.1
      );
      if (AzraqPlotting[i].commercial === true) {
      ...
      if (AzraqPlotting[i].type === "Medium") {
        temp.rotation.set(0, 0, -AzraqPlotting[i].rotation.radian + 0.3);
      } else {
        temp.rotation.set(0, 0, -AzraqPlotting[i].rotation.radian);
      }
    ...
    console.log("azsmall: ", small);
    console.log("azmedium: ", medium);
    console.log("az large: ", large);
  }, []);
  return (
    <instancedMesh
      ref={ref}
      args={[null, null, AzraqPlotting.length]}
      onClick={(e) => {
        e.stopPropagation();
        const temp = new THREE.Object3D();
        for (let i = 0; i < AzraqPlotting.length; i++) {
          if (i === e.instanceId) {
            console.log(AzraqPlotting[i]);
            //   if (AzraqPlotting[i].name.includes("small")) {
            //     temp.scale.set(0.35, 0.35, 5);
            //   } else if (AzraqPlotting[i].name.includes("Medium")) {
