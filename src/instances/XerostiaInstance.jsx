...
import { XerostiaPlotting } from "../plots/XerostiaPlotting";

export function XerostiaInstances({ temp = new THREE.Object3D() }) {
  const ref = useRef();
  ...
  useEffect(() => {
    for (let i = 0; i < XerostiaPlotting.length; i++) {
      if (XerostiaPlotting[i].type === "Small") {
        small++;
        temp.scale.set(0.35, 0.35, 0.35);
      } else if (XerostiaPlotting[i].type === "Medium") {
        medium++;
        temp.scale.set(0.67, 0.67, 0.67);
      } else if (XerostiaPlotting[i].type === "XL") {
        temp.scale.set(2, 2, 2);
      } else if (XerostiaPlotting[i].type === "XXL") {
      ...
      temp.position.set(
        XerostiaPlotting[i].position.x,
        XerostiaPlotting[i].position.y,
        4.1
      );
      if (XerostiaPlotting[i].commercial === true) {
      ...
      temp.rotation.set(0, 0, -XerostiaPlotting[i].rotation.radian);
      temp.updateMatrix();
    ...
    // console.log("Total: ", XerostiaPlotting.length);
  }, []);
  return (
    <instancedMesh
      ref={ref}
      args={[null, null, XerostiaPlotting.length]}
      // onClick={(e) => {
      //   e.stopPropagation();
